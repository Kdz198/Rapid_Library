...
// import { FaBell, FaChartBar, FaExchangeAlt, FaHome, FaPiggyBank, FaSignInAlt, FaSignOutAlt, FaTags, FaUser, FaUserPlus } from "react-icons/fa";
...
//       if (user && user.id) {
//         try {
//           const response = await fetch(`http://localhost:8080/api/account/findbyid?id=${user.id}`, { method: "GET", credentials: "include" });
//           if (!response.ok) {
//             const errorData = await response.json();
//             throw new Error(`Không thể tải dữ liệu tài khoản: ${response.status} - ${errorData.message || "Lỗi không xác định"}`);
//           }
...
//       if (user && user.id) {
//         try {
//           const response = await fetch(`http://localhost:8080/api/notification?id=${user.id}`, { method: "GET", credentials: "include" });
//           if (!response.ok) {
//             const errorData = await response.json();
//             throw new Error(`Không thể tải thông báo: ${response.status} - ${errorData.message || "Lỗi không xác định"}`);
//           }
//           const data = await response.json();
...
//     try {
//       const response = await fetch(`http://localhost:8080/api/notification/read?notificationId=${notificationId}`, {
//         method: "GET",
//         credentials: "include",
//       });
//       if (!response.ok) {
//         const errorData = await response.json();
//         throw new Error(`Không thể đánh dấu thông báo là đã đọc: ${response.status} - ${errorData.message || "Lỗi không xác định"}`);
//       }
//       setNotifications((prev) =>
//         prev.map(
//           (noti) => (noti.id === notificationId ? { ...noti, read: true } : noti) // Sử dụng read
//         )
//       );
...
//   const avatarUrl = accountData && accountData.gender ? maleAvatar : femaleAvatar;
...
//             <Link to={isLoggedIn ? "/dashboard" : "/"} className="text-3xl font-bold text-white">
//               Smart Wallet
//             </Link>
//           </div>
...
//                             className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${!noti.read ? "bg-blue-50" : ""}`}
//                           >
//                             <p className="text-sm font-medium text-gray-800">{noti.tittle || "Thông báo mới"}</p>
//                             <p className="text-xs text-gray-500">{noti.message || "Không có nội dung"}</p>
//                           </div>
//                         ))}
//                       </div>
//                       <div className="p-4">
...
//                 >
//                   <img className="h-8 w-8 rounded-full" src={avatarUrl} alt="Avatar người dùng" />
...
//                 <img className="h-10 w-10 rounded-full" src={avatarUrl} alt="Avatar người dùng" />
//                 <div>
//                   <div className="font-medium text-gray-800">{accountData.name}</div>
