...
import { Book, Calendar, ChevronDown, ChevronUp, RefreshCw, Search } from "lucide-react";
...
function HistoryPage() {
    ...
    if (user?.email) {
      ...
      axios
        .get("http://localhost:8080/api/loans", {
          headers: {
            Authorization: `Bearer ${token}`,
            stool: "application/json",
            "Access-Control-Allow-Origin": "*",
            "ngrok-skip-browser-warning": "true",
          },
        })
        .then((response) => {
          const userLoans = response.data.filter((loan) => loan.userEmail === user.email);
          setLoans(userLoans);
        })
        .catch((error) => {
  ...
  const filteredLoans = loans.filter((loan) => {
    ...
    const matchesStatus = selectedStatus === "All" || loan.status === (selectedStatus === "Borrowed" ? "BORROWED" : "RETURNED");

    const matchesDate = !selectedDate || new Date(loan.borrowDate).toDateString() === selectedDate.toDateString();
  ...
  const handleReturnBook = async (loanId) => {
    ...
    try {
      ...
      setLoans((prevLoans) =>
        prevLoans.map((loan) => (loan.id === loanId ? { ...loan, status: "RETURNED", returnDate: new Date().toISOString().split("T")[0] } : loan))
      );
    } catch (error) {
      ...
      <div className="backdrop-blur-xl bg-white/70 rounded-xl p-4 shadow-md border border-white/30">
            ...
            <DatePicker
              ...
              renderCustomHeader={({ date, decreaseMonth, increaseMonth, prevMonthButtonDisabled, nextMonthButtonDisabled }) => (
                <div className="flex items-center justify-between px-2 py-1">
                  ...
                  <span className="text-xs font-semibold text-slate-800">{date.toLocaleString("default", { month: "long", year: "numeric" })}</span>
                  <button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-4">
            ...
            <h3 className="text-sm font-semibold text-slate-800 mb-1">No Borrowing History</h3>
            <p className="text-xs text-slate-500">No loans match your search or filter criteria.</p>
          </div>
        ) : (
            ...
            {filteredLoans.map((loan) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 cursor-pointer flex-grow" onClick={() => toggleLoanDetails(loan.id)}>
                ...
                {expandedLoan === loan.id && (
                  <div className="mt-2 pl-2 text-xs text-slate-600 animate-fadeIn w-full">
                    <p>
                      <span className="font-semibold">Borrow Date:</span> {loan.borrowDate}
                    </p>
                    <p>
                      <span className="font-semibold">Due Date:</span> {loan.dueDate}
                    </p>
                    {loan.returnDate && (
                      <p>
                        <span className="font-semibold">Return Date:</span> {loan.returnDate}
                      </p>
                    )}
                    <p>
                      <span className="font-semibold">Books:</span> {getBookTitles(loan.books)}
                    </p>
                  </div>
                )}
              </div>
